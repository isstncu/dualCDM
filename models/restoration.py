...
class DiffusiveRestoration:
    ...
    def restore(self, test_loader, image_dirs, validation='test', r=None):
        image_folder = os.path.join(self.args.image_folder)
        with torch.no_grad():
            for i, (x,filename) in enumerate(test_loader):
                print('x:',x.shape)
                ...
                x_cond = x.to(self.diffusion.device)
                x_output = self.diffusive_restoration(x_cond, r=r)
                print(x_output.shape)
                
                utils.logging.save_image(x_output, os.path.join(image_folder, f"PRED_{y}.tif"))
    ...
    def diffusive_restoration(self, x_cond, r=None):
        ...
        batch,band,patch_sizeA,patch_sizeB=x_cond.size()
        x = torch.randn(batch,4,patch_sizeA,patch_sizeB, device=self.diffusion.device)
