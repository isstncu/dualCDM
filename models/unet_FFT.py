...
def get_timestep_embedding(timesteps, embedding_dim, downscale_freq_shift=0, flip_sin_to_cos=True):
    """
...
def Normalize_complex(in_channels):
    return ComplexBatchNorm2d(in_channels)
...
class DiffusionUNet(nn.Module):
    def __init__(self, config):
        ...
        for i_level in reversed(range(self.num_resolutions)): 
            block = nn.ModuleList()
            attn = nn.ModuleList()
            #crattn = nn.ModuleList()
            block_out = ch*ch_mult[i_level] 
            skip_in = ch*ch_mult[i_level] 
            for i_block in range(self.num_res_blocks+1): 
                if i_block == self.num_res_blocks: 
                    skip_in = ch*in_ch_mult[i_level] 
                #crattn.append(CrossAttention(skip_in))
    ...
    def forward(self, x, t=None):
        ...
        for i_level in range(self.num_resolutions):
            for i_block in range(self.num_res_blocks):
                h = self.down_complex[i_level].block_complex[i_block](hs_fft[-1])
                hs_fft.append(h)
            if i_level != self.num_resolutions-1:
                hs_fft.append(self.down_complex[i_level].downsample(hs_fft[-1]))

        hs_fft_C = torch.fft.ifftn(hs_fft[-1], dim=(-2, -1))
        hs_fft_C = torch.real(hs_fft_C)
        ...
        for i_level in reversed(range(self.num_resolutions)):
            for i_block in range(self.num_res_blocks+1):
                #feature = self.up[i_level].crattn[i_block](hs.pop(), torch.real(torch.fft.ifftn(hs_fft.pop(), dim=(-2, -1))))
                h = self.up[i_level].block[i_block](
                    torch.cat([h, (hs.pop()+torch.real(torch.fft.ifftn(hs_fft.pop(), dim=(-2, -1))))], dim=1), temb) #pop删除并返回最后一个元素
                    #torch.cat([h, feature], dim=1), temb)
